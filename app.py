...
def create_docx(isim, sections_list):
    ...
    doc.add_paragraph(f"Davacı: {isim if isim else '..........'}")
...
with st.sidebar:
    ...
    # Placeholder kullanımı (Silik metin)
    isim = st.text_input("İşçinin Adı Soyadı", value="", placeholder="Ad Soyad giriniz...")
    
    # Tarihler varsayılan olarak boş (None) veya bugünkü tarih seçilebilir
    g_tarih = st.date_input("İşe Giriş Tarihi", value=None)
    c_tarih = st.date_input("İşten Çıkış Tarihi", value=None)
    
    # Sayısal değerler 0.00 olarak başlar
    son_brut = st.number_input("Brüt Ücret (TL)", min_value=0.0, value=0.0, step=100.0, format="%.2f")
    yemek_ucreti = st.number_input("Yemek Ücreti (Aylık Brüt)", min_value=0.0, value=0.0, step=10.0, format="%.2f")
    fm_saat = st.number_input("Haftalık Fazla Mesai Saati", min_value=0.0, value=0.0, step=0.5)
    izin_gun = st.number_input("Kalan İzin Günü", min_value=0, value=0)
    ubgt_yillik = st.number_input("Yıllık Çalışılan UBGT Günü", min_value=0, value=0)

if st.button("HESAPLA VE DETAYLI RAPORU OLUŞTUR", type="primary"):
    # Tarih kontrolü
    if g_tarih is None or c_tarih is None:
        st.error("Lütfen İşe Giriş ve İşten Çıkış tarihlerini seçiniz.")
    elif son_brut <= 0:
        st.warning("Lütfen Brüt Ücret bilgisini giriniz.")
    else:
        # --- HESAPLAMALAR ---
        ...
        d_asgari_cikis = ASGARI_UCRET_TABLOSU.get(c_tarih.year, 17002.12)
        ...
        for y in range(g_tarih.year, c_tarih.year + 1):
            d_maas = ASGARI_UCRET_TABLOSU.get(y, 17002.12) * maas_orani
            sz = (d_maas / 225) * 1.5
        ...
        ub_brut, ub_rows = 0, []
        for y in range(g_tarih.year, c_tarih.year + 1):
            d_m = ASGARI_UCRET_TABLOSU.get(y, 17002.12) * maas_orani; gu = d_m / 30
            bas = max(g_tarih, datetime(y, 1, 1).date()); bit = min(c_tarih, datetime(y, 12, 31).date())
        ...
        # WORD İNDİRME
        st.download_button(label="📥 Raporu Word Olarak İndir", data=create_docx(isim, word_sections), file_name=f"Bilirkişi_Raporu_{isim if isim else 'Yeni'}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
