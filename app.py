...
if st.button("HESAPLA VE RAPORU OLUŞTUR", type="primary"):
    # 1. TEMEL PARAMETRELER VE ÜCRET TESPİTİ
    ...
    st.table(pd.DataFrame([
        ...
        ["**TOPLAM**", "", "", "", f"**{format_tl(b_kidem)}**"],
        ["Damga Vergisi", "Binde 7,59", "", "=", format_tl(k_res['dv'])],
        ["**Net Kıdem Tazminatı**", "", "", "", f"**{format_tl(k_res['net'])}**"]
    ]))
    ...
    st.table(pd.DataFrame([
        ...
        ["**Net İhbar Tazminatı**", "", "", "=", f"**{format_tl(i_res['net'])}**"]
    ]))

    # 3. YILLIK İZİN (DETAYLI)
    ...
    for y in range(g_tarih.year, c_tarih.year + 1):
        ...
        fm_brut_total += b_fm
        fm_rows.append([f"{y} Dönemi", f"{format_tl(d_asgari)} x {maas_orani:.4f} / 225 x 1,5", f"= {format_tl(saatlik)}", format_tl(b_fm)])
    
    st.table(pd.DataFrame(fm_rows, columns=["Çalışma Dönemi", "Saatlik Zamlı Ücret Hesabı", "Sonuç", "Brüt Tutar"]))
    fm_res = kesinti_hesapla(fm_brut_total)
    
    ubgt_brut_total = (son_brut / 30) * ubgt_yillik * yil
    u_res = kesinti_hesapla(ubgt_brut_total)

    # 5. SONUÇ VE İCMAL TABLOSU (YENİ EKLENEN)
    st.markdown("### 7. Sonuç ve İcmal (ÖZET) Tablosu")
    st.info("Hesaplanan tüm alacakların brüt ve net dökümü:")
    ...
    st.success(f"**Toplam Net Alacak: {format_tl(k_res['net']+i_res['net']+z_res['net']+fm_res['net']+u_res['net'])}**")
