...
if st.button("HESAPLA VE DETAYLI RAPORU OLUŞTUR", type="primary"):
    ...
    ucret_data = [["Brüt Ücret", format_tl(son_brut)], ["Yemek Ücreti", format_tl(yemek_ucreti)], ["Giydirilmiş Brüt Ücret", format_tl(giydirilmis_brut)], ["Kıdem Tazminatına Esas Ücret", format_tl(esas_kidem_ucret)], ["Asgari Ücrete Oranı", "{:.5f}".format(maas_orani).replace(".", ",")]]
    st.table(pd.DataFrame(ucret_data, columns=["Kalem", "Miktar"]))

    # --- 4. KIDEM VE İHBAR ---
    st.markdown("### 4. Kıdem ve İhbar Tazminatının Hesaplanması")
    k_yil_t = esas_kidem_ucret * yil; k_ay_t = (esas_kidem_ucret / 12) * ay; k_gun_t = (esas_kidem_ucret / 365) * gun
    b_kidem = k_yil_t + k_ay_t + k_gun_t; k_res = kesinti_hesapla(b_kidem, "kıdem")
    st.markdown("**Kıdem Tazminatı:**")
    st.table(pd.DataFrame([[format_tl(esas_kidem_ucret), "x", f"{yil} Yıl", "=", format_tl(k_yil_t)], [f"{format_tl(esas_kidem_ucret)} / 12", "x", f"{ay} Ay", "=", format_tl(k_ay_t)], [f"{format_tl(esas_kidem_ucret)} / 365", "x", f"{gun} Gün", "=", format_tl(k_gun_t)], ["**TOPLAM**", "", "", "", f"**{format_tl(b_kidem)}**"], ["Damga Vergisi", "Binde 7,59", "", "=", format_tl(k_res['dv'])], ["**NET KIDEM**", "", "", "", f"**{format_tl(k_res['net'])}**"]]))

    b_ihbar = (son_brut / 30) * 7 * ihbar_hafta; i_res = kesinti_hesapla(b_ihbar)
    st.markdown(f"**İhbar Tazminatı ({ihbar_hafta} Hafta):**")
    ...
    b_izin = (son_brut / 30) * izin_gun; z_res = kesinti_hesapla(b_izin)
    ...
    # --- 6. FAZLA MESAİ (KESİNTİLER DAHİL) ---
    ...
    for y in range(g_tarih.year, c_tarih.year + 1):
        ...
        fm_brut_total += b_fm
        fm_rows.append({"Dönem": f"{y}", "Saatlik Ücret Hesabı": f"({format_tl(d_maas)} / 225) * 1,5 = {format_tl(zamli_saatlik)}", "FM Süresi Hesabı": f"{fm_saat} Saat * {h_say:.2f} Hafta", "Brüt Tutar": format_tl(b_fm)})
    st.table(pd.DataFrame(fm_rows))
    fm_res = kesinti_hesapla(fm_brut_total)
    
    st.markdown("**Fazla Mesai Kesinti Dökümü:**")
    st.table(pd.DataFrame([["Brüt Toplam", "=", format_tl(fm_brut_total)], ["SGK Primi (%15)", "=", format_tl(fm_res['sgk'])], ["Gelir Vergisi (%15)", "=", format_tl(fm_res['gv'])], ["Damga Vergisi (Binde 7,59)", "=", format_tl(fm_res['dv'])], ["**Net Fazla Mesai**", "=", f"**{format_tl(fm_res['net'])}**"]]))

    # --- 6.1 UBGT (KESİNTİLER DAHİL) ---
    st.markdown("### 6.1 UBGT Ücretinin Hesaplanması")
    ubgt_brut_total = 0
    ubgt_rows = []
    for y in range(g_tarih.year, c_tarih.year + 1):
        d_asgari = ASGARI_UCRET_TABLOSU.get(y, 25000.0); d_maas = d_asgari * maas_orani; gunluk = d_maas / 30
        bas = max(g_tarih, datetime(y, 1, 1).date()); bit = min(c_tarih, datetime(y, 12, 31).date())
    ...
    st.table(pd.DataFrame([["Brüt Toplam", "=", format_tl(ubgt_brut_total)], ["SGK Primi (%15)", "=", format_tl(u_res['sgk'])], ["Gelir Vergisi (%15)", "=", format_tl(u_res['gv'])], ["Damga Vergisi (Binde 7,59)", "=", format_tl(u_res['dv'])], ["**Net UBGT**", "=", f"**{format_tl(u_res['net'])}**"]]))
    ...
    icmal_data = [["Kıdem Tazminatı", format_tl(b_kidem), format_tl(k_res['toplam_kesinti']), format_tl(k_res['net'])], ["İhbar Tazminatı", format_tl(b_ihbar), format_tl(i_res['toplam_kesinti']), format_tl(i_res['net'])], ["Yıllık İzin Ücreti", format_tl(b_izin), format_tl(z_res['toplam_kesinti']), format_tl(z_res['net'])], ["Fazla Mesai Ücreti", format_tl(fm_brut_total), format_tl(fm_res['toplam_kesinti']), format_tl(fm_res['net'])], ["UBGT Ücreti", format_tl(ubgt_brut_total), format_tl(u_res['toplam_kesinti']), format_tl(u_res['net'])], ["**GENEL TOPLAM**", "**" + format_tl(g_brut) + "**", "**" + format_tl(g_kesinti) + "**", "**" + format_tl(g_net) + "**"]]
